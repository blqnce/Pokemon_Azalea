...
import net.falconidos.pokemonazalea.block.entity.custom.ElekidEntity;
...
public class ElekidPokeballBlock extends BlockWithEntity implements BlockEntityProvider {

    public static final MapCodec<ElekidPokeballBlock> CODEC = createCodec(ElekidPokeballBlock::new);
    ...
    public ElekidPokeballBlock(Settings settings) {
        super(settings);
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            if (be instanceof ElekidEntity elekid) {

                MinecraftServer server = world.getServer();
                CommandManager commandManager = server.getCommandManager();
                commandManager.executeWithPrefix(player.getCommandSource().withLevel(4).withSilent(), "playsound pokemonazalea:elekid_cry player " + player.getName().getString());

                player.openHandledScreen(elekid);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new ElekidEntity(pos, state);
    }
