...
import net.falconidos.pokemonazalea.block.entity.custom.MudkipEntity;
...
public class MudkipPokeballBlock extends BlockWithEntity implements BlockEntityProvider {

    public static final MapCodec<MudkipPokeballBlock> CODEC = createCodec(MudkipPokeballBlock::new);
    ...
    public MudkipPokeballBlock(Settings settings) {
        super(settings);
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            if (be instanceof MudkipEntity mudkip) {

                MinecraftServer server = world.getServer();
                CommandManager commandManager = server.getCommandManager();
                commandManager.executeWithPrefix(player.getCommandSource().withLevel(4).withSilent(), "playsound pokemonazalea:mudkip_cry player " + player.getName().getString());

                player.openHandledScreen(mudkip);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new MudkipEntity(pos, state);
    }
