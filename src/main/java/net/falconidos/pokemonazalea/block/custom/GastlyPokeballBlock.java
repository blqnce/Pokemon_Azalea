...
import net.falconidos.pokemonazalea.block.entity.custom.GastlyEntity;
...
public class GastlyPokeballBlock extends BlockWithEntity implements BlockEntityProvider {

    public static final MapCodec<GastlyPokeballBlock> CODEC = createCodec(GastlyPokeballBlock::new);
    ...
    public GastlyPokeballBlock(Settings settings) {
        super(settings);
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            if (be instanceof GastlyEntity gastly) {

                MinecraftServer server = world.getServer();
                CommandManager commandManager = server.getCommandManager();
                commandManager.executeWithPrefix(player.getCommandSource().withLevel(4).withSilent(), "playsound pokemonazalea:gastly_cry player " + player.getName().getString());

                player.openHandledScreen(gastly);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new GastlyEntity(pos, state);
    }
