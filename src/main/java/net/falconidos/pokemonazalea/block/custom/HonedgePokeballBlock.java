...
import net.falconidos.pokemonazalea.block.entity.custom.HonedgeEntity;
...
public class HonedgePokeballBlock extends BlockWithEntity implements BlockEntityProvider {

    public static final MapCodec<HonedgePokeballBlock> CODEC = createCodec(HonedgePokeballBlock::new);
    ...
    public HonedgePokeballBlock(Settings settings) {
        super(settings);
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            if (be instanceof HonedgeEntity honedge) {

                MinecraftServer server = world.getServer();
                CommandManager commandManager = server.getCommandManager();
                commandManager.executeWithPrefix(player.getCommandSource().withLevel(4).withSilent(), "playsound pokemonazalea:honedge_cry player " + player.getName().getString());

                player.openHandledScreen(honedge);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new HonedgeEntity(pos, state);
    }
