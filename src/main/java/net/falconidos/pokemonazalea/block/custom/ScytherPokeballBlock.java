...
import net.falconidos.pokemonazalea.block.entity.custom.ScytherEntity;
...
public class ScytherPokeballBlock extends BlockWithEntity implements BlockEntityProvider {

    public static final MapCodec<ScytherPokeballBlock> CODEC = createCodec(ScytherPokeballBlock::new);
    ...
    public ScytherPokeballBlock(Settings settings) {
        super(settings);
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            if (be instanceof ScytherEntity scyther) {

                MinecraftServer server = world.getServer();
                CommandManager commandManager = server.getCommandManager();
                commandManager.executeWithPrefix(player.getCommandSource().withLevel(4).withSilent(), "playsound pokemonazalea:scyther_cry player " + player.getName().getString());

                player.openHandledScreen(scyther);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new ScytherEntity(pos, state);
    }
