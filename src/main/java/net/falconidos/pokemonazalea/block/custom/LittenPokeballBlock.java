...
import net.falconidos.pokemonazalea.block.entity.custom.LittenEntity;
...
public class LittenPokeballBlock extends BlockWithEntity implements BlockEntityProvider {

    public static final MapCodec<LittenPokeballBlock> CODEC = createCodec(LittenPokeballBlock::new);
    ...
    public LittenPokeballBlock(Settings settings) {
        super(settings);
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            if (be instanceof LittenEntity litten) {

                MinecraftServer server = world.getServer();
                CommandManager commandManager = server.getCommandManager();
                commandManager.executeWithPrefix(player.getCommandSource().withLevel(4).withSilent(), "playsound pokemonazalea:litten_cry player " + player.getName().getString());

                player.openHandledScreen(litten);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new LittenEntity(pos, state);
    }
