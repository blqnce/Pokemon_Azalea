...
import net.falconidos.pokemonazalea.block.entity.custom.VulpixEntity;
...
public class VulpixPokeballBlock extends BlockWithEntity implements BlockEntityProvider {

    public static final MapCodec<VulpixPokeballBlock> CODEC = createCodec(VulpixPokeballBlock::new);
    ...
    public VulpixPokeballBlock(Settings settings) {
        super(settings);
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            if (be instanceof VulpixEntity vulpix) {

                MinecraftServer server = world.getServer();
                CommandManager commandManager = server.getCommandManager();
                commandManager.executeWithPrefix(player.getCommandSource().withLevel(4).withSilent(), "playsound pokemonazalea:vulpix_cry player " + player.getName().getString());

                player.openHandledScreen(vulpix);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new VulpixEntity(pos, state);
    }
