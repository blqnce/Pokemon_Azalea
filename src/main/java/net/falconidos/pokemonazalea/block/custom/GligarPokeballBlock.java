...
import net.falconidos.pokemonazalea.block.entity.custom.GligarEntity;
...
public class GligarPokeballBlock extends BlockWithEntity implements BlockEntityProvider {

    public static final MapCodec<GligarPokeballBlock> CODEC = createCodec(GligarPokeballBlock::new);
    ...
    public GligarPokeballBlock(Settings settings) {
        super(settings);
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            if (be instanceof GligarEntity gligar) {

                MinecraftServer server = world.getServer();
                CommandManager commandManager = server.getCommandManager();
                commandManager.executeWithPrefix(player.getCommandSource().withLevel(4).withSilent(), "playsound pokemonazalea:gligar_cry player " + player.getName().getString());

                player.openHandledScreen(gligar);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new GligarEntity(pos, state);
    }
