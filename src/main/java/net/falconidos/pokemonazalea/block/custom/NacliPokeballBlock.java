...
import net.falconidos.pokemonazalea.block.entity.custom.NacliEntity;
...
public class NacliPokeballBlock extends BlockWithEntity implements BlockEntityProvider {

    public static final MapCodec<NacliPokeballBlock> CODEC = createCodec(NacliPokeballBlock::new);
    ...
    public NacliPokeballBlock(Settings settings) {
        super(settings);
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            if (be instanceof NacliEntity nacli) {

                MinecraftServer server = world.getServer();
                CommandManager commandManager = server.getCommandManager();
                commandManager.executeWithPrefix(player.getCommandSource().withLevel(4).withSilent(), "playsound pokemonazalea:nacli_cry player " + player.getName().getString());

                player.openHandledScreen(nacli);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new NacliEntity(pos, state);
    }
