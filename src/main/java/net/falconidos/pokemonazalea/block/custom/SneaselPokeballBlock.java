...
import net.falconidos.pokemonazalea.block.entity.custom.SneaselEntity;
...
public class SneaselPokeballBlock extends BlockWithEntity implements BlockEntityProvider {

    public static final MapCodec<SneaselPokeballBlock> CODEC = createCodec(SneaselPokeballBlock::new);
    ...
    public SneaselPokeballBlock(Settings settings) {
        super(settings);
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            if (be instanceof SneaselEntity sneasel) {

                MinecraftServer server = world.getServer();
                CommandManager commandManager = server.getCommandManager();
                commandManager.executeWithPrefix(player.getCommandSource().withLevel(4).withSilent(), "playsound pokemonazalea:sneasel_cry player " + player.getName().getString());

                player.openHandledScreen(sneasel);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new SneaselEntity(pos, state);
    }
